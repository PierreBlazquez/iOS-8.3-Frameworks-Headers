//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "PBCodable.h"

#import "NSCopying.h"

@interface WifiParameters : PBCodable <NSCopying>
{
    int _batching;
    float _historyTimespanSeconds;
    float _scanPeriodSeconds;
    struct {
        unsigned int batching:1;
        unsigned int historyTimespanSeconds:1;
        unsigned int scanPeriodSeconds:1;
    } _has;
}

- (void)copyTo:(id)arg1;
- (void)writeTo:(id)arg1;
- (BOOL)readFrom:(id)arg1;
- (void)mergeFrom:(id)arg1;
@property(nonatomic) int batching; // @synthesize batching=_batching;
@property(nonatomic) float scanPeriodSeconds; // @synthesize scanPeriodSeconds=_scanPeriodSeconds;
@property(nonatomic) float historyTimespanSeconds; // @synthesize historyTimespanSeconds=_historyTimespanSeconds;
@property(nonatomic) BOOL hasBatching;
@property(nonatomic) BOOL hasScanPeriodSeconds;
@property(nonatomic) BOOL hasHistoryTimespanSeconds;
- (id)dictionaryRepresentation;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (id)description;
- (unsigned int)hash;
- (BOOL)isEqual:(id)arg1;

@end

